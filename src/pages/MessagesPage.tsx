...
const ZAP_AGENT_CONTACT: Contact = {
    id: 1, name: "ZAP AGENT", status: 'online', lastSeen: 'NOW',
    lastMessage: "Awaiting your command, Operator...", time: "LIVE", unread: 1, role: 'AI AGENT'
};

const ZAP_SUPPORT_CONTACT: Contact = {
    id: 2, name: "ZAP SUPPORT HQ", status: 'offline', lastSeen: '8H AGO',
    lastMessage: "Ticket #9432: VPR validated. +50 ZP added...", time: "8h ago", unread: 0, role: 'OFFICIAL'
};

const MOCK_CONTACTS: Contact[] = [ZAP_AGENT_CONTACT, ZAP_SUPPORT_CONTACT];

const INITIAL_ZAP_AGENT_HISTORY: Message[] = [
    { id: 1, sender: 'them', text: "ZAP AGENT online. I am your tactical assistant for the ZAP ecosystem. How can I help you gain an edge?", time: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) }
];
...
export const MessagesPage = () => {
    ...
    const handleSendMessage = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const response = await ai.models.generateContent({
                ...
                config: { systemInstruction: 'You are Zap Agent, an AI assistant for the ZAP platform specializing in crypto gambling intel. Your tone is tactical, concise, and futuristic. Keep responses brief and use markdown for formatting.' }
            });

            const aiMessage: Message = {
                id: Date.now() + 1, sender: 'them', text: response.text,
                time: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })
            };
            ...
            const errorMessage: Message = {
                id: Date.now() + 1, sender: 'them', text: 'CRITICAL ERROR: Connection to the Grid severed.',
                time: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })
            };
    ...
    return (
        <div className="container mx-auto max-w-7xl h-full p-4 md:p-8 page-fade-in flex flex-col">
            <div className="mb-6 flex-shrink-0">
                <h1 className="font-heading text-2xl md:text-3xl font-bold text-white uppercase tracking-wider flex items-center gap-3">
                    <Icons.MessageSquare className="h-6 w-6 md:h-8 md:h-8 text-[#00FFC0] animate-pulse-slow" />
                    SECURE COMM LINK
                </h1>
                <p className="text-[#00FFC0] font-mono text-xs md:text-sm uppercase tracking-widest mt-1 ml-9 md:ml-12 text-glow">
                ...
                <div className={`w-full md:w-1/3 lg:w-1/4 bg-[#0c0c0e] border-r border-[#333] flex flex-col transition-all duration-300 absolute inset-0 z-10 md:relative md:z-auto ${mobileView === 'chat' ? '-translate-x-full md:translate-x-0' : 'translate-x-0'}`}>
                    ...
                    <div className="flex-1 overflow-y-auto custom-scrollbar">
                        {MOCK_CONTACTS.map(contact => (
                            <button key={contact.id} onClick={() => handleContactSelect(contact.id)}
